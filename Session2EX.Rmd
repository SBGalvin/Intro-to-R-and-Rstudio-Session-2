...
We can use the `by()` function to group data and summarise it with a function (`descr()`) from the `summarytools` package
```{r}
by(df.long,                # Tell R where the data is
     df.long$Condition,    # Tell R how you want to group the data, (by the factor variable)
     descr)                # Tell R what function you want to use
```
...
```{r}
NC  <- read.csv("NicCage.csv", fileEncoding="UTF-8-BOM")
NC$Year
```
```{r}
